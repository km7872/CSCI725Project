...
def check_if_review_exist(user, product_id, cur):
    query = "SELECT * FROM reviews WHERE user = %s AND product_id = %s"
...
def post_reviews():
    ...
    while count != 20000:
        ...
        if not check_if_review_exist(username, product_id, cur):
            rating = random.uniform(1.0, 5.0)
            review_text = "Random review text"
            # check if user product entry does not already exist
            insert_query = 'insert into reviews (reviews_description, reviews_rating, date, "user", product_id) ' \
                           'values (%s, %s, %s, %s, %s)'
            try:
                cur.execute(insert_query, (
                review_text, rating, date.today(), username, product_id))
                conn.commit()
                count += 1
            except Exception as e:
                conn.rollback()
...
def submit_past_orders():
    ...
    while count != 10000:
        ...
        product_quantities = [
            (random.choice(product_id_list), random.randint(1, 5)) for i in
            range(10)]
        ...
        try:
            ...
            order_values = ", ".join(
                ["(%s, %s, %s)" % (order_id, product_id, quantity) for
                 product_id, quantity in product_quantities])
            insert_items_query = 'insert into items (order_id, product_id, product_quantity) values {0}'.format(
                order_values)
