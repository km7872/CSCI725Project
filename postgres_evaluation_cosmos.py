...
def check_if_user_exist(user, cur):
...
def CreateAccount(username, password, firstName, lastName):
    ...
    cur = conn.cursor()
    if not check_if_user_exist(username, cur):
...
def PostReview(username, password, productID, rating, reviewText):
    ...
    cur.execute(check_cred, (username, password))
    result = cur.fetchall()
    if result and not check_if_review_exist(username,productID, cur):
